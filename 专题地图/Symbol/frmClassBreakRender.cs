...
{
    public partial class frmClassBreakRender : Form
    {
        List<IFeatureClass> _lstFeatCls = null;
        public delegate void ClassBreakRenderEventHandler(string sFeatClsName, string sFieldName, int intClsBreakNum);
        public event ClassBreakRenderEventHandler ClassBreakRender = null;
        public frmClassBreakRender()
        {
            InitializeComponent();
        }
        private IMap _map;
        ...
        private bool check()
        {
            ...
            if (cmbClassNumber.SelectedIndex == -1)
            {
               MessageBox.Show("请选择划分级别！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                return false;
            }
            return true;
        }
        ...
        {
            if (!check()) return;
            ClassBreakRender(cmbSelLyr.SelectedItem.ToString(),
                cmbSelField.SelectedItem.ToString(),
                Convert.ToInt32(cmbClassNumber.SelectedItem.ToString()));
            cmbSelField.Items.Clear();
            cmbSelField.Text = "";
            cmbSelLyr.Items.Clear();
            cmbSelLyr.Text = "";
