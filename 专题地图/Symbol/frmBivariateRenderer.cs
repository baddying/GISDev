...
{
    public partial class frmBivariateRenderer : Form
    {
        List<IFeatureClass> _lstFeatCls = null;
        public delegate void BivariateRendererEventHandler(string sFeatClsName, string sFieldName ,int numclasses);
        public event BivariateRendererEventHandler BivariateRenderer = null;
        public frmBivariateRenderer()
        {
            InitializeComponent();
        }
        private IMap _map;
        ...
        {
            if (!check()) return;
            BivariateRenderer(cmbSelLyr.SelectedItem.ToString(), cmbSelField.SelectedItem.ToString(), Convert.ToInt32(cmbnumclasses.SelectedItem.ToString()));
            cmbSelField.Items.Clear();
            cmbSelField.Text = "";
            cmbSelLyr.Items.Clear();
            cmbSelLyr.Text = "";
