...
{
    public partial class FrmXYToPoint : Form
    {
       ...
       /// </summary>
        ...
        private void btnOpenExcel_Click(object sender, EventArgs e)
        {
            ...
            txtExcelPath.Text = strDBName;
            txtExcelPath.Tag = OFD.FilterIndex;
            //构造连接Excel字符串
            ...
            foreach (System.Data.DataRow row in dtTable.Rows)
            {
                ...
                cmbExcelSheets.Items.Add(strTableName);
            }
        ...
        {
            this.cmbXField.Items.Clear();
            this.cmbYField.Items.Clear();
            // 选择sheet表发生变化时，读取sheet表中列名加载到“成图X字段”和“成图Y字段”
            //下拉选择列表
            string sheetTableName = cmbExcelSheets.SelectedItem.ToString();
           ...
           dt= QueryBySql(String.Format("select top 10 * from [{0}]", sheetTableName));
            if(dt != null)
            {
                ...
                {
                    this.cmbXField.Items.Add(column.ColumnName);    
                    this.cmbYField.Items.Add(column.ColumnName);                 
                }
                this.cmbXField.SelectedIndex = 0;
                this.cmbYField.SelectedIndex = 0;
            }
        }
        ...
        private void btnConvert_Click(object sender, EventArgs e)
        {
            ...
            if (this.txtExcelPath.Text.Trim() ==string.Empty)
            {
                MessageBox.Show("坐标数据不能为空！");
                return;
            }
            if (this.cmbExcelSheets.SelectedIndex == -1)
            {
                MessageBox.Show("未选择X Y坐标数据Sheet表！");
                return;
            }
            if (this.cmbXField.SelectedIndex == -1)
            {
                MessageBox.Show("未选择成图X字段 ！");
                return;
            }
            if (this.cmbYField.SelectedIndex == -1)
            {
                MessageBox.Show("未选择成图Y字段 ！");
                return;
            }
            if (this.cmbXField.SelectedItem.ToString() ==this.cmbYField.SelectedItem.ToString() )
            {
                MessageBox.Show("成图X字段与成图Y字段不能相同 ！");
                return;
            }
            //下面进行数据成图
            string xFieldName = this.cmbXField.SelectedItem.ToString();
            string yFieldName = this.cmbYField.SelectedItem.ToString();
            string sheetTableName = this.cmbExcelSheets.SelectedItem.ToString();
            //获得所有数据
